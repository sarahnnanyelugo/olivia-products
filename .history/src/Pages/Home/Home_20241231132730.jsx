...
import { Offcanvas } from "react-bootstrap"; // Bootstrap's offcanvas
...
export const Home = () => {
  ...
  const [cart, setCart] = useState([]); // State to manage the cart globally
  ...
  const handleAddToCart = (item) => {
    setCart((prevCart) => {
      ...
      if (!itemExists) {
        return [...prevCart, item]; // Add item if not in the cart
      }
      return prevCart; // Return cart as-is if item already exists
    });
  };
