...
export const Home = () => {
  ...
  // Prepare the duplicated products for seamless looping
  ...
  // Calculate the actual start index in the extended array
  ...
  // Reset the position after the transition ends (loop effect)
  useEffect(() => {
    if (isTransitioning) {
      const timeout = setTimeout(() => {
        setIsTransitioning(false);

        // Reset to the real position without transition
        if (startIndex === -1) {
  ...
  return (
    <div className="home-div">
      ...
      <center>
        ...
        >
          <div
            className="carousel row row-cols-2 row-cols-lg-4 g-2 g-lg-3"
            style={{
              display: "flex",
          ...
          >
            {extendedProducts.map((product, index) => (
              <div
                key={index}
                style={{
                  flex: `0 0 ${100 / visibleItems}%`, // Set item width dynamically
