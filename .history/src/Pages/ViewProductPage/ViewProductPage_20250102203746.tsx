...
// Define the type for the media items
interface Media {
  type: "image" | "video"; // Specify the media types
  src: string;
}
...
interface Product {
  ...
  name: string;
  description: string;
  additionalMedia?: Media[]; // Add the optional additionalMedia property
}
...
export const ViewProductPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setProduct(selectedProduct || null); // Ensure it's either a Product or null
  }, [id]);
  ...
  // Define images array, filter out only 'image' type from additionalMedia
  const images = [
    product.firstImg,
    product.hoverImg,
