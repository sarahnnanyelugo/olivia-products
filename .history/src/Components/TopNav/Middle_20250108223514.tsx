...
export const Middle: React.FC = () => {
  ...
  const megaMenuRef = useRef<HTMLDivElement>(null); // Ref for the mega menu itself
  ...
  useEffect(() => {
    if (isOpen && menuRef.current && megaMenuRef.current) {
      const menuRect = menuRef.current.getBoundingClientRect();
      megaMenuRef.current.style.top = `${menuRect.bottom + 120}px`;
    }
  }, [isOpen]);
